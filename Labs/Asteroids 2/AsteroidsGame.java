...
public class AsteroidsGame
{
    ...
    if (scoreboard.getLives() > 0)
    {
      ...
      for (int i = 0; i < asteroids.size(); i++)
      {
        ...
        for (int j = 0; j < bullets.size(); j++)
        {
          ...
          {
            if(a instanceof ToughAsteroid) {
              ((ToughAsteroid) a).weGotHit();
              if(((ToughAsteroid) a).isHealthOne()) {
                asteroids.remove(i);
              }
            } else {
              bullets.remove(j);
  ...
  private void spawnAsteroids()
  {
    ...
    {
      for (int i = 0; i < GameConstants.DEFAULT_ASTEROIDS_PER_LEVEL - 1; i++)
      {
        Asteroid asteroid = new Asteroid();
      ...
      Asteroid toughAsteroid = new ToughAsteroid();
      asteroids.add(toughAsteroid);
    }
  }
}
